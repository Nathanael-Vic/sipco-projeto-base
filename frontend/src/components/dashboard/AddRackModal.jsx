...
export default function AddRackModal({ project, onClose, onSuccess }) {
  const [rackForm, setRackForm] = useState({
    nome: '', coluna: 1, linha: 1, capacidade: 42, ocupado: 0, temperatura: 22, potencia: 0
  });
  ...
  const handleSaveRack = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/racks`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          temperatura_c: parseFloat(rackForm.temperatura),
          potencia_kw: parseFloat(rackForm.potencia)
        })
      });
    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalBox}>
        ...
        <form onSubmit={handleSaveRack}>
          ...
          <div style={styles.row}>
            <div style={{flex:1}}><label style={styles.label}>Temperatura (°C)</label><input type="number" step="0.1" style={styles.input} value={rackForm.temperatura} onChange={e => setRackForm({...rackForm, temperatura: e.target.value})} /></div>
            <div style={{flex:1}}><label style={styles.label}>Potência (kW)</label><input type="number" step="0.1" style={styles.input} value={rackForm.potencia} onChange={e => setRackForm({...rackForm, potencia: e.target.value})} /></div>
          </div>
