...
const DataHallMap = ({ racks = [], eletrocalhas = [], width, height, projectWidth, projectHeight, stageState, onWheel, onDragEnd }) => {
  ...
  const displayWidth = projectWidth + PADDING;
  const displayHeight = projectHeight + PADDING;
  ...
  const renderEmptySlots = () => {
    ...
    for (let i = 1; i <= cols; i++) {
      for (let j = 1; j <= rows; j++) {
        ...
        slots.push(
          <Group key={`slot-${i}-${j}`} x={(i-1) * gridSize + PADDING/2} y={(j-1) * gridSize + PADDING/2}>
  ...
  return (
    ...
    >
      <Layer>
        ...
        {racks.map(rack => {
          const xPos = (rack.coordenada_x - 1) * gridSize + PADDING/2;
          const yPos = (rack.coordenada_y - 1) * gridSize + PADDING/2;
          ...
          return (
            <Group key={`rack-${rack.id}`} x={xPos} y={yPos}>
        ...
        {eletrocalhas.map(calha => {
          const x1 = (calha.ponto_a_x - 1) * gridSize + PADDING/2;
          const y1 = (calha.ponto_a_y - 1) * gridSize + PADDING/2;
          const x2 = (calha.ponto_b_x - 1) * gridSize + PADDING/2;
          const y2 = (calha.ponto_b_y - 1) * gridSize + PADDING/2;

          if (x1 < 0) return null;

          return <Line key={`calha-${calha.id}`} points={[x1+gridSize/2, y1+gridSize/2, x2+gridSize/2, y2+gridSize/2]} stroke={Theme.colors.calha} strokeWidth={6} lineCap="round" opacity={0.7} />;
        })}

        {overlapDots.map(dot => <Circle key={dot.key} x={dot.x + PADDING/2} y={dot.y + PADDING/2} radius={4} fill={Theme.colors.dot} stroke="#fff" strokeWidth={1} />)}
      </Layer>
    </Stage>
  );
};
